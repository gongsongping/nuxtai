...
export default {
  ...
  methods: {
    ...
    checkCss(f) {
      ...
      if ($(f.path).get(0) === undefined) {
        alert(f.name + "字段css选择器无效");
        f.path = "";
        return;
      }
      if (f.type === "image") {
        ...
        let last = f.path
          ...
          .slice(0, 3);
        if (last !== "img") {
          alert(f.name + "图片选择器最后一个元素应该为img");
          f.path = "";
          return;
        }
      }
    ...
    async beginScrawl() {
      ...
      if (!this.rule.detailurl) {
        alert("详情页url不能为空");
        return;
      }
      if (!this.rule.listurl) {
        alert("列表页url不能为空");
        return;
      }
      ...
      this.rule.fields.forEach(function(f, i) {
        if (!f.name) {
          alert("第" + (i + 1) + "字段名字不能为空");
          return;
        }
        if (!f.path) {
          alert("第" + (i + 1) + "字段css选择器不能为空");
          return;
        }

        // if (document.querySelector(f.path)===null) {alert('第'+(i+1)+'字段css选择器无效'); return}
        if ($(f.path).get(0) === undefined) {
          alert("第" + (i + 1) + "字段css选择器无效");
          return;
        }
        if (f.type === "image") {
          if (f.path.indexOf("> img") < 0) {
            alert("第" + (i + 1) + "字段css选择器里没有图片image");
            return;
          }
        }
        if (f.type === "link") {
          if (f.path.indexOf("> a") < 0) {
            alert("第" + (i + 1) + "字段css选择器里没有有效链接a");
            return;
          }
        }
      });
